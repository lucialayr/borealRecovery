...
df = read_csv("data/processed/trajectories_ssp585_2015_2040_timeseries_rf.csv")

##1. Calculate length of transient
...
get_veggie_data = function(s, start_year, end_year) {
  ...
  df_c2 = df_dominant %>%
     ...
     select(Lon, Lat, PID, length_transient) %>%
     unique()  %>%
  ...
  df = df_dominant %>%
    select(Lon, Lat, PID, year_disturbance) %>%
    unique() %>%
    full_join(df_c1) %>%
    full_join(df_c2) %>%
    full_join(df_c3) %>%
...
get_climate_data = function(df, s, start_age, window, start_year, end_year) {
  ...
  df = df %>%
    select(Lat, Lon, PID, year_disturbance, class, s, length_transient) %>%
    ungroup() %>%
...
create_data = function(start_year, end_year) {
  ...
  for (s in c("ssp585", "ssp126", "picontrol")) {
    ...
    df = get_veggie_data(s, start_year, end_year) %>%
...
create_plot = function(start_year, end_year) {
  ...
  cairo_pdf(paste0("figures/results/logistic_model_", start_year, "_", end_year, ".pdf"), width = 7.5, height = 7) 
  print(p)
  dev.off()
  ...
  cairo_pdf(paste0("figures/results/logistic_model_", start_year, "_", end_year, "_linear.pdf"), width = 7.5, height = 7) 
  print(p)
  dev.off()
...
p1 = create_plot(2015, 2040)
p2 = create_plot(2075, 2100)
...
histograms_length_transient = function() {
  ...
  ggsave("figures/results/histogram_transient_length.png", width = 10)
...
location_of_classes = function(start_year, end_year) {
  ...
  (p1 = ggplot() + theme_bw() +
      ...
      geom_sf(data = df_sf[df_sf$class == 2, ], color = "darkgrey", size = .005, shape = 20) +
      geom_sf(data = df_sf[df_sf$class == 0, ], aes(color = s), size = .005, shape = 20) +
      ...
      ggnewscale::new_scale_color() +
      geom_sf(data = df_sf[df_sf$class == 1, ], aes(color = s), size = .005, shape = 20) +
      scico::scale_color_scico_d(palette = "lajolla", begin = .2, end = .8, name = "Decidous transient \n> 10 years", direction = -1) +
      scale_x_continuous(expand = c(0,0)) +
      scale_y_continuous(expand = c(0,0)) +
      #scale_fill_manual(name = "Scenario", values = c("Control" = "#F0BD57", "SSP1-RCP2.6" = "#D85F4D", "SSP5-RCP8.5" = "#512C1E",
      #                                                      "Study region\noutside\nrealized niche" = "grey"),
