...
validation_A_plot = function() {
  ...
  shp = st_read(here("data", "final", "shp", "validation_A.shp"))
...
validation_B_plot = function() {
  ...
  df = read_csv(here("data", "final", "validation_B.csv")) %>%
    mutate(PFT = ifelse(PFT == "Shrubs, Herb. & Sparse Veg.", "Shrubs & Herb.", PFT))
  ...
  (p2 = ggplot() +
      ...
      facet_grid(rows = vars(PFT), cols = vars(ecoregion), scales = 'free_y') +
      ...
      geom_point(data = df, aes(x = age_bin, y = value, fill = ecoregion, shape = dataset), size = 3, color = "black") +  # Adding color for outline
      scale_x_discrete(name = "Year since disturbance") +
      scale_y_continuous(name = "Relative share of vegetation cover", 
                         breaks = c(0.25, 0.5, 0.75),  labels = c('0.25', "", '0.75'), limits = c(0, 1), expand = c(0,0)) +
      scale_fill_scico_d(name = "Ecoregions", palette = "navia", begin = .1, direction = 1) +
      ...
      theme(legend.position = "bottom",
            ...
            axis.text.x = element_text(angle = 90, size = 11, vjust = .5)) +
      guides(fill = 'none',
...
validation_plot = function() {
  ...
  plot_grid(plot_grid(validation_A, legend, ncol = 1, rel_heights = c(1, 0.1)),
            validation_B + theme(legend.position = 'none'), 
            labels = c("(a)", "(b)"), rel_widths = c(0.625, 1),  axis = "b")
  
  ggsave(here("plots", "recovery_validation.pdf"), width = 11)
  ggsave(here("plots", "recovery_validation.png"), width = 10, height = 6.5, dpi = 300)
